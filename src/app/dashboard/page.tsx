...
export default function Dashboard() {
  ...
  const openEditDialog = (link: any) => {
    setEditingLink(link);
    setEditTargetUrl(link.original_url);
    setEditOpen(true);
  };
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                links.map((link) => (
                  <TableRow key={link.id} className="border-slate-800 hover:bg-muted/30 transition-colors">
                    ...
                    <TableCell className="text-muted-foreground">
                      <div className="max-w-[300px] truncate" title={link.original_url}>
                        {link.original_url}
                      </div>
                    </TableCell>
